...
| Co-Trainer hinzufügen/entfernen (`FC-ROLE-002`/`FC-ROLE-003`) | `grant_assistant_coach()`/`revoke_assistant_coach()` prüfen `team_owner` und lehnen Self-Targeting serverseitig ab. Nur `grant_assistant_coach()` verlangt eine nachweisbare, aktive Spielerbeziehung des Zielnutzers (Eligibility für die Vergabe); `revoke_assistant_coach()` prüft dies bewusst nicht, da ein Entzug auch nach Spielerentfernung möglich bleiben muss. Beide sind gegeneinander per Row-Lock auf der Mitgliedschaftszeile serialisiert; die Mitgliedschaft wird nur deaktiviert, wenn nach dem Entzug weder eine Rolle noch eine aktive Spielerbeziehung verbleibt. `list_assistant_coaches()` ist ebenfalls `team_owner`-only. Alle drei mit explizitem `REVOKE`/`GRANT EXECUTE`. `team_role_audit_log` ist nur per `SELECT`-RLS für `team_owner` lesbar, kein Schreibgrant für `authenticated`. | `supabase/migrations/20260723100000_add_role_management.sql`, `tests/e2e/core-flow-role-management.spec.ts` |
